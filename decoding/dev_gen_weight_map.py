...
def load_data(strategy: str, preprocessing: str, data_path: str, plot: bool=False):
    ...
    conds_fmri = conds_fmri.iloc[list(range(fmri_data.shape[3]))]  # cut onsets to brain data (cut off last scale tr's that are variable and cannot be read from onsets.txt)
...
def get_conds_from_txt(onsets, cond_names, tr):
    ...
    for i in range(10): # todo: relevant for pilots 3 and 4, can be removed later: append 10 rows at the end of the data frame, to correct for the unsaved final onset
...
# todo: check todo's - 1 base for pandas cond file (check whether it runs for pilot 2); check whether all runs for pilots 3 and 4
